// app.js — FINAL BACKEND + POMODORO VERSION
// ================================================
...
(function () {
    ...
    list.forEach((c) => {
      ...
      li.innerHTML = `
        ...
        <div class="text-end">
          ...
          <div class="mt-2">
            <button class="btn btn-sm btn-outline-secondary me-1" data-action="edit-course" data-id="${c.id}">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" data-action="del-course" data-id="${c.id}">
  ...
  async function renderSessions() {
    ...
    const courseMap = Object.fromEntries((resCourses.data || []).map((c) => [c.id, c]));
    ...
    sessions.forEach((s) => {
      ...
      tr.innerHTML = `
        ...
        <td>${s.note || ''}</td>
        <td class="text-end">
            <button class="btn btn-sm btn-outline-secondary" data-action="edit-session" data-id="${s.id}">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger" data-action="del-session" data-id="${s.id}">
              <i class="bi bi-trash"></i>
            </button>
        </td>
      `;
  ...
  async function renderSummaryAndCharts() {
    ...
    const totalMinutes = weekSessions.reduce((sum, s) => sum + Number(s.effective_minutes), 0);
    ...
    const rate = totalSessionMinutes === 0 ? 0 : Math.round((totalMinutes / totalSessionMinutes) * 100);
    ...
    const totals = courses
      .map((c) => {
        const sum = sessions.filter((s) => s.course_id == c.id).reduce((a, b) => a + Number(b.effective_minutes) / 60, 0);
  ...
  function updatePomoDisplay() {
    ...
    el.pomoDisplay.textContent = String(m).padStart(2, '0') + ':' + String(s).padStart(2, '0');

    el.pomoMode.textContent = pomoMode === 'focus' ? 'Mode: Fokus' : 'Mode: Istirahat';
  }
  ...
  // INIT
  // ======================================================
  document.getElementById('modalAddSession').addEventListener('show.bs.modal', function () {
    el.sessionId.value = '';
    el.formSession.reset();
  });
